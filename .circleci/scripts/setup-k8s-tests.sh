...
WITH_CRIO="${WITH_CRIO:-false}"
if [ "$WITH_CRIO" = "true" ]; then
    K8S_VERSION="$K8S_MAX_VERSION"
elif [ -z "$K8S_VERSION" ]; then
    echo "K8S_VERSION not defined!"
    exit 1
fi
...
if [ "$CIRCLE_BRANCH" != "master" ] && ! scripts/changes-include-dir $CHANGES_INCLUDE; then
    ...
    if [[ "$WITH_CRIO" != "true" && "$K8S_VERSION" != "$K8S_MIN_VERSION" && "$K8S_VERSION" != "$K8S_MAX_VERSION" ]]; then
